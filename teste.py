...
nome_arquivo_formatado = 'N13_Final_5.0.xlsx'

inicio_export = time.time()

writer = pd.ExcelWriter(nome_arquivo_formatado, engine='xlsxwriter', engine_kwargs={'options': {'constant_memory': True}})
...
for i in range(0, total_linhas, tamanho_chunk):
                ...
                print(f"Salvando linhas no Excel: {linhas_processadas:,} de {total_linhas:,} concluídas...")

# Salva o DataFrame na aba 'Valoracao'
df_n13p_ordenado.to_excel(writer, sheet_name='Valoracao', index=False)

workbook  = writer.book
worksheet = writer.sheets['Valoracao']

# # 1. Ativa as setas de filtro automático em todas as colunas da planilha de uma vez só!
worksheet.autofilter(0, 0, total_linhas, len(df_n13p_ordenado.columns) - 1)

# # 2. Exibe as linhas de grade (gridlines) cinzas normais do Excel para facilitar a leitura visual das linhas
...
# ==============================================================================
# 4. APLICAÇÃO DOS FORMATOS, ESTILOS E AUTO-AJUSTE DE LARGURA
# ==============================================================================
for col_num, col_nome in enumerate(df_n13p_ordenado.columns):
    
    # 1. Definição do formato visual do cabeçalho (Box do N13P vs Outras colunas)
    if col_nome == col_n13p_fim:
        formato_cabecalho = fmt_header_n13p_fim
    else:
        # Outras Colunas (Sem bordas laterais nos cabeçalhos)
        if col_nome in col_roxo:
    ...
    print(f"📏 Formatação do cabeçalho da coluna {col_nome} aplicada com sucesso! {formato_cabecalho}")
