...
def _formatar_e_gerar_excel(df_n13p, ciclo):
    ...
    try:
        periodo = int(ciclo['periodo'])
        ano = int(ciclo['ano'])
        nome_arquivo_temporario = f'N13_P{periodo:02d}_{ano}.xlsx'


        print("🚀 Formatando os dados...")

        with pd.ExcelWriter(nome_arquivo_temporario, engine='xlsxwriter') as writer:
            ...
            df_n13p_ordenado.to_excel(writer, sheet_name='Valoracao', index=False, startrow=9, header=False)

        with open(nome_arquivo_temporario, 'rb') as f:
            excel_data = f.read()
        ...
        raise Exception(f"Erro ao declarar formatação: {str(e)}")
